...
impl Theme {
    ...
    /// Built-ins follow the default's contrast pattern: a saturated selection bar
    /// with dark text on it, muted text readable on a dark background, dim still
    /// visible. Palette "surface"/"comment" shades are deliberately not used.
